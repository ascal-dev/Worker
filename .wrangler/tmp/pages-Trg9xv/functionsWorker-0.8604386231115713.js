...
async function onRequestPost(context) {
  try {
    ...
    const { data, error } = await supabase.from("xnxxvideos").update({
      stream_url,
      img,
      // Update image as well if provided (user logic: fix img based on stream)
      post_date_raw: (/* @__PURE__ */ new Date()).toISOString()
      // Optional: touch the record
    }).eq("id", id).select();
    if (error) {
      throw error;
    }
...
async function onRequest2(context) {
  ...
  if (extractMode === "kpm") {
    ...
    try {
      ...
      const postMatch = searchHtml.match(/<h2 class="entry-title"><a href="([^"]+)"/i) || searchHtml.match(/<a class="item-link" href="([^"]+)"/i);
      ...
      const match2 = html.match(/video_url['":\s]+['"]([^'"]+\.(mp4|m3u8)[^'"]*)['"]/i) || html.match(/<source[^>]+src=['"]([^'"]+\.(mp4|m3u8)[^'"]*)['"]/i) || html.match(/file['":\s]+['"]([^'"]+\.(mp4|m3u8)[^'"]*)['"]/i) || html.match(/["'](https?:\/\/[^"']+\.m3u8[^"']*)["']/i) || html.match(/["'](https?:\/\/[^"']+\.mp4[^"']*)["']/i);
      if (match2) {
        let v = match2[1].replace(/\\u/g, "%u").replace(/\\\//g, "/");
  ...
  if (extractVideo && (target.includes("noodlemagazine.com") || target.includes("xhamster.desi") || target.includes("eroticmv.com") || target.includes("fullxcinema.com") || target.includes("viralfap.com"))) {
    try {
      ...
      const match2 = html.match(/video_url['":\s]+['"]([^'"]+\.(mp4|m3u8)[^'"]*)['"]/i) || html.match(/<source[^>]+src=['"]([^'"]+\.(mp4|m3u8)[^'"]*)['"]/i) || html.match(/file['":\s]+['"]([^'"]+\.(mp4|m3u8)[^'"]*)['"]/i) || html.match(/["'](https?:\/\/[^"']+\.m3u8[^"']*)["']/i) || html.match(/["'](https?:\/\/[^"']+\.mp4[^"']*)["']/i) || html.match(/["']\/wp-content\/[^"']+\.mp4[^"']*["']/i) || html.match(/"videoUrl"\s*:\s*"([^"]+)"/i) || html.match(/property="og:video"\s+content=["']([^"']+)["']/i);
      if (match2) {
        let v = match2[1].replace(/\\u/g, "%u").replace(/\\\//g, "/");
  ...
  try {
    const response = await fetch(target, {
      ...
      redirect: "manual",
      // Handle redirects manually for cookie/url control
      cf: { cacheTtl: 0 }
    });
...
async function handleProxyResponse(upstream, target, cors) {
  ...
  passAttrs.forEach((a) => {
    if (upstream.headers.has(a)) respHeaders[a] = upstream.headers.get(a);
  });
  ...
  if (contentType.includes("text/html")) {
    ...
    html = html.replace(
      /style=["']([^"']*url\([^)]+\)[^"']*)["']/gi,
      (m, s) => 'style="' + s.replace(/url\(["']?([^"')]+)["']?\)/gi, (um, u) => 'url("' + pU(u.trim()) + '")') + '"'
    );
    const stealthScript = `
<script>
(function() {
...
__name(handleProxyResponse, "handleProxyResponse");

// ../.wrangler/tmp/pages-Trg9xv/functionsRoutes-0.5493324777086459.mjs
var routes = [
  {
...
// C:/Users/Sachin/AppData/Roaming/npm/node_modules/wrangler/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// C:/Users/Sachin/AppData/Roaming/npm/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
